...
def create(request):
    ...
    if request.method == 'POST':
        create=crudForm(request.POST, request.FILES) #berfungsi untuk menangkap data bertipe file
        if create.is_valid():
            create.save()
...
def update(request,up_id):
    ...
    update=crudForm(request.POST or None, request.FILES or None, initial=updateForm, instance=update_id)
